...
export class TicksToDatePipe implements PipeTransform {
  transform(value: number | undefined): string {
    if (value !== undefined) {
      ...
      let days = Math.round(time / (24 * 60 * 60 * 10e6)); // Math.floor() rounds a number downwards to the nearest whole integer, which in this case is the value representing the day
      let hours = Math.floor((time / (60 * 60 * 10e6)) % 24); // Math.round() rounds the number up or down
